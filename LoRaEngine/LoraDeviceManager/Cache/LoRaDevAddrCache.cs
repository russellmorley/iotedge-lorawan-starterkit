...
namespace LoraDeviceManager.Cache
{
    ...
    public sealed class LoRaDevAddrCache
    {
        ...
        public LoRaDevAddrCache(ICacheStore cacheStore, IDeviceRegistryManager registryManager, ILogger logger, string gatewayId)
        {
            this.cacheStore = cacheStore;
            this.logger = logger ?? NullLogger.Instance;
            lockOwner = gatewayId ?? Guid.NewGuid().ToString();
        ...
        /// </summary>
        public LoRaDevAddrCache(ICacheStore cacheStore, ILogger logger, string gatewayId)
        {
            this.cacheStore = cacheStore;
            this.logger = logger ?? NullLogger.Instance;
            lockOwner = gatewayId ?? Guid.NewGuid().ToString();
        ...
        {
            var tmp = cacheStore.GetHashObject(GenerateKey(devAddr));
            if (tmp?.Length > 0)
            {
        ...
        public void StoreInfo(DevAddrCacheInfo info)
        {
            ...
            cacheStore.SetHashObject(cacheKeyToUse, subKey, serializedObjectValue);
        ...
        public async Task PerformNeededSyncs(IDeviceRegistryManager registryManager)
        {
            // If a full update is expected
            if (await cacheStore.LockTakeAsync(FullUpdateLockKey, lockOwner, FullUpdateKeyTimeSpan, block: false))
            {
                ...
                try
                {
                    ...
                    if (ownsUpdateLock = await cacheStore.LockTakeAsync(UpdatingDevAddrCacheLock, lockOwner, UpdatingDevAddrCacheLockTimeSpan, block: true))
                    {
                        logger.LogDebug("A full reload was started");
                        await PerformFullReload(registryManager);
                        logger.LogDebug("A full reload was completed");
                        ...
                        _ = cacheStore.TryChangeLockTTL(FullUpdateLockKey, FullUpdateKeyTimeSpan);
                        fullUpdatePerformed = true;
                    }
                    else
                    {
                        logger.LogDebug("A full reload was needed but failed to acquire global update lock");
                    }
                }
                catch (RedisException ex)
                {
                    logger.LogError($"Exception occured during dev addresses full reload {ex}");
                }
                finally
                {
                    if (ownsUpdateLock)
                    {
                        ...
                        _ = cacheStore.LockRelease(UpdatingDevAddrCacheLock, lockOwner);
                    }

                    if (!fullUpdatePerformed)
                    {
                        if (!cacheStore.TryChangeLockTTL(FullUpdateLockKey, timeToExpire: TimeSpan.FromMinutes(1)))
                        {
                            logger.LogError("Could not change the TTL of the lock");
                        }
                    }
                }
            }
            else if (await cacheStore.LockTakeAsync(UpdatingDevAddrCacheLock, lockOwner, TimeSpan.FromMinutes(5), block: false))
            {
                try
                {
                    logger.LogDebug("A delta reload was started");
                    await PerformDeltaReload(registryManager);
                    logger.LogDebug("A delta reload was completed");
                }
                catch (RedisException ex)
                {
                    logger.LogError($"Exception occured during dev addresses delta reload while interacting with Redis {ex}");
                }
                catch (IotHubCommunicationException ex)
                {
                    logger.LogError($"Exception occured during dev addresses delta reload during communication with the iot hub {ex}");
                }
                catch (IotHubException ex)
                {
                    logger.LogError($"An unknown IoT Hub exception occured during dev addresses delta reload  {ex}");
                }
                finally
                {
                    _ = cacheStore.LockRelease(UpdatingDevAddrCacheLock, lockOwner);
        ...
        public async Task<TimeSpan?> GetTTLInformation(string key) => await cacheStore.GetObjectTTL(key);
        ...
        private async Task PerformDeltaReload(IDeviceRegistryManager registryManager)
        {
            ...
            var lastUpdate = long.TryParse(cacheStore.StringGet(LastDeltaUpdateKeyValue), out var cachedTicks) ? cachedTicks : DateTime.UtcNow.AddHours(-1).Ticks;
        ...
        private async Task<List<DevAddrCacheInfo>> GetDeviceTwinsFromIotHub(IRegistryPageResult<ILoRaDeviceTwin> query, long? lastDeltaUpdateFromCacheTicks)
        {
            ...
            while (query.HasMoreResults)
            {
                ...
                {
                    if (!twin.Properties.Desired.TryRead(TwinPropertiesConstants.DevAddr, logger, out DevAddr devAddr) &&
                        !twin.Properties.Reported.TryRead(TwinPropertiesConstants.DevAddr, logger, out devAddr))
                    {
                        continue;
                    }
            ...
            if (!isFullReload)
            {
                _ = cacheStore.StringSet(LastDeltaUpdateKeyValue, lastDeltaUpdateFromCacheTicks.Value.ToString(CultureInfo.InvariantCulture), TimeSpan.FromDays(1));
            }
        ...
        private void BulkSaveDevAddrCache(List<DevAddrCacheInfo> devAddrCacheInfos, bool canDeleteDeviceWithDevAddr)
        {
            ...
            foreach (var elementPerDevAddr in regrouping)
            {
                ...
                var currentDevAddrEntry = cacheStore.GetHashObject(cacheKey);
                ...
                {
                    cacheStore.ReplaceHashObjects(cacheKey, devicesByDevEui, DevAddrObjectsTTL, canDeleteDeviceWithDevAddr);
                }
            }
        }
        ...
        {
            return await cacheStore.LockTakeAsync(string.Concat(DevAddrLockName, devAddr), lockOwner, DefaultSingleLockExpiry, block: true);
        }

        public bool ReleaseDevAddrUpdateLock(DevAddr devAddr)
        {
            return cacheStore.LockRelease(string.Concat(DevAddrLockName, devAddr), lockOwner);
        }
    }
}
